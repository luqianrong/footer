...
public class WheelView extends ScrollView {
    ...
    public static class OnWheelViewListener {
        public void onSelected(int selectedIndex, SelectItem item) {
        }
...
//    private ScrollView scrollView;
    ...
    private List<? extends SelectItem> getItems() {
        return items;
    }
        ...
        for (Object item : items) {
            String itemStr="";
             if (item instanceof SelectItem){
                itemStr=((SelectItem) item).name;
            }
            views.addView(createView(itemStr));
        }
...
//            position = divided + offset + 1;
        ...
        for (int i = 0; i < childSize; i++) {
            ...
            if (position == i) {
                itemView.setTextColor(Color.parseColor("#2ca6e5"));//选中的文本的颜色
            } else {
                itemView.setTextColor(Color.parseColor("#b8b8b8"));//没选中的文本的颜色
            }
        }
    }
    ...
    public void setBackgroundDrawable(Drawable background) {
        ...
        if (null == paint) {
            paint = new Paint();
            paint.setColor(Color.parseColor("#d8d8d8"));//设定两条线的颜色#83cde6
            paint.setStrokeWidth(ABTextUtil.dip2px(context, 1f));//ABTextUtil.dip2px(context, 1f)
        }
